...
import har.makawet.jvm.Utils.ImagePickerr;
...
public class ChatUserActivity extends AppCompatActivity {
    ...
    private ImageView imgvBlockUser,imgvRemoveMessages;
    private View mViewInflatedialogLogout;
    private DatabaseReference dbBlock;
    private ArrayList<String> arrMessagesKeys;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        imgvBlockUser.setVisibility(View.VISIBLE);
        ...
        imgvBlockUser.setOnClickListener(new View.OnClickListener() {
            @Override
        ...
        imgvRemoveMessages.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Log.d(TAG, "onClick: array "+arrMessagesKeys);
                for (int i=0;i<arrMessagesKeys.size();i++){
                    if(!arrMessagesKeys.get(i).equals("a")){
                        int finalI = i;
                        dbrefMyChat.child(arrMessagesKeys.get(i)).addListenerForSingleValueEvent(new ValueEventListener() {
                            @Override
                            public void onDataChange(@NonNull DataSnapshot snapshot) {
                                if (snapshot.child("from").getValue().equals(userID)){
                                    dbrefMyChat.child(arrMessagesKeys.get(finalI)).removeValue();
                                    dbrefHischat.child(arrMessagesKeys.get(finalI)).removeValue();

                                }else{
                                    dbrefMyChat.child(arrMessagesKeys.get(finalI)).removeValue();
                                }
                                arrMessagesKeys.set(finalI,"a");
                                imgvRemoveMessages.setVisibility(View.GONE);
                            }
    ...
    private void dialogBlock(){
        ...
        final AlertDialog.Builder alertDialogBuilder = DialogUtils.CustomAlertDialog(mViewInflatedialogLogout, ChatUserActivity.this);
        final AlertDialog alertDialog = alertDialogBuilder.create();
        ...
        tv_dialog_logout_confirm.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                dbBlock.child(userID).child(hisID).child("blocked").setValue("true");
                dbBlock.child(hisID).child(userID).child("blocked").setValue("true");
    ...
    private void widgets() {
        ...
        imgvBlockUser = findViewById(R.id.imgvBlockUser);
    ...
    private void sendMessage(String message,String type){
        ...
        dbrefHischat.child(keypuch).child("from").setValue(userID);
        dbrefHischat.child(keypuch).child("message").setValue(message);
        dbrefHischat.child(keypuch).child("type").setValue(type);
    ...
    // this is useed for fill array with a latter for change place a to place position message
    private void loadingArrayMessageEmpty(ArrayList<String> stringArrayList){
        for (int i=1; i<50 ; i++){
    ...
    private void firebaseRecyclerview(){
       ...
       arrMessagesKeys =  new ArrayList<>();
        loadingArrayMessageEmpty(arrMessagesKeys);
        ...
        chatRecyclerAdapte = new FirebaseRecyclerAdapter<Chat, chatViewHolder>(
            ...
            protected void populateViewHolder(final chatViewHolder viewHolder, Chat model, int position) {
                viewHolder.lymymessage.setBackground(null);
                ...
                dbrefMyChat.child(list_msg_id).addValueEventListener(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        ...
                        viewHolder.lymymessage.setOnLongClickListener(new View.OnLongClickListener() {
                            @Override
                            public boolean onLongClick(View v) {


                                Log.d(TAG, "onLongClick current is position  "+ position);

                                        if (arrMessagesKeys.get(position).equals(list_msg_id)) {
                                            viewHolder.lymymessage.setBackground(null);
                                           // arrMessagesKeys.add(position,"a");
                                            arrMessagesKeys.set(position,"a");
                                           // arrMessagesKeys.remove(position);

                                            Log.d(TAG, "onLongClick remove backgr is position  "+ position);
                                        } else {
                                            viewHolder.lymymessage.setBackgroundColor(getResources().getColor(R.color.colorTransparentRemove));
                                            arrMessagesKeys.set(position,list_msg_id);
                                            imgvRemoveMessages.setVisibility(View.VISIBLE);
                                            Log.d(TAG, "onLongClick add backgr is position  "+ position);

                                        }

                                Log.d(TAG, "space/////////////////");

                                Log.d(TAG, "onLongClick: "+arrMessagesKeys.toString());
